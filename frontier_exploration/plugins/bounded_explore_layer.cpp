...
namespace frontier_exploration
{
    ...
    void BoundedExploreLayer::onInitialize(){
        ...
        nh_.param<double>("preferred_travel_point_distance", preferred_travel_point_distance_, 0.0);
        nh_.param<int>("min_frontier_size", min_frontier_size_, 1);
        nh_.param<std::string>("global_frame", global_frame_, "map");
        nh_.param<double>("blacklist_radius", blacklist_radius_, 1.0);
    ...
    bool BoundedExploreLayer::getNextFrontier(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped &next_frontier){
        ...
        FrontierSearch frontierSearch(*(layered_costmap_->getCostmap()), preferred_travel_point_distance_, min_frontier_size_, frontier_travel_point_);
        //get list of frontiers from search implementation
        std::list<Frontier> frontier_list = frontierSearch.searchFrom(start_pose.pose.position, yawOfQuaternion(start_pose.pose.orientation));
