...
FrontierSearch::FrontierSearch(costmap_2d::Costmap2D &costmap, double preferred_travel_point_distance, int min_frontier_size, std::string &travel_point) :
    costmap_(costmap), preferred_travel_point_distance_(preferred_travel_point_distance), min_frontier_size_(min_frontier_size), travel_point_(travel_point) { }

std::list<Frontier> FrontierSearch::searchFrom(geometry_msgs::Point position, double orientation){
    ...
    while(!bfs.empty()){
        ...
        BOOST_FOREACH(unsigned nbr, nhood4(idx, costmap_)){
            ...
            if(map_[nbr] <= map_[idx] && !visited_flag[nbr]){
                ...
                Frontier new_frontier = buildNewFrontier(nbr, pos, orientation, frontier_flag);
...
Frontier FrontierSearch::buildNewFrontier(unsigned int initial_cell, unsigned int reference, double orientation, std::vector<bool>& frontier_flag){

    //initialize frontier structure
    Frontier output;
    geometry_msgs::Point centroid, forward, middle;
    output.size = 1;
    output.min_distance = std::numeric_limits<double>::infinity();
    double min_forward_distance = std::numeric_limits<double>::infinity();
    ...
    while(!bfs.empty()){
        ...
        BOOST_FOREACH(unsigned int nbr, nhood8(idx, costmap_)){
            ...
            //find angle difference. Make sure -pi <= angle_difference <= pi
            double angle_difference = fmod(orientation - atan2(wy - reference_y, wx - reference_x) + 5 * M_PI, 2 * M_PI) - M_PI;
            //determine frontier's forward distance using a metric which prefers forward movement and little turning
            double forward_distance = abs(distance - preferred_travel_point_distance_) + 1.5 * pow(angle_difference, 2.0);
            ...
            if(isNewFrontierCell(nbr,frontier_flag)){
                ...
                bfs.push(nbr);
            } else if (map_[nbr] == NO_INFORMATION && !frontier_flag[nbr] && distance <= preferred_travel_point_distance_) {
                //unknown cell within the preferred travel point distance. Use as candidate for forward movement.
                //mark cell as frontier
                frontier_flag[nbr] = true;
    ...
    }else if(travel_point_ == "forward"){
        output.travel_point = forward;
    }else{
