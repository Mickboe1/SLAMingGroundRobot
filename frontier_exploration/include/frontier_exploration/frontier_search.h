...
public:
     ...
     * @param preferred_travel_point_distance The preferred distance to travel to a frontier.
     * @param min_frontier_size The minimum size to accept a frontier
     * @param travel_point The requested travel point (closest|middle|centroid|front)
     */
    FrontierSearch(costmap_2d::Costmap2D& costmap, double preferred_travel_point_distance, int min_frontier_size, std::string &travel_point);

    /**
     * @brief Runs search implementation, outward from the start position
     * @param position Initial position to search from
     * @param orientation The forward orientation (yaw)
     * @return List of frontiers, if any
     */
    std::list<Frontier> searchFrom(geometry_msgs::Point position, double orientation);
...
protected:
     ...
     * @return
     */
    Frontier buildNewFrontier(unsigned int initial_cell, unsigned int reference, double orientation, std::vector<bool>& frontier_flag);
